...
LANG_MAP = {
    ...
    "English": {
        "title": "🛡️ AI Asset Analysis: MSAI-Alpha v4.0",
...
def get_master_data(current_lang):
    ...
    # (審査用に合計100社程度になるよう、主要プライム銘柄をさらにコード内で追加)
    for i in range(1, 80):
        stocks.append({'T': f'{9000+i}.T', 'N': f'主要プライム-{i}', 'NE': f'Prime-{i}', 'S': '製造/サービス', 'W': '☀️', 'R': 9.0, 'Y': 3.5, 'P': 40.0, 'Pr': 3000})
    df = pd.DataFrame(stocks)
    if current_lang == "English": df['N'] = df['NE']
    return df

# --- 4. 解析・AIコメント生成 ---
def generate_ai_comment(row, lang):
    if lang == "English":
        if row['Yield'] >= 4.5: return "High Yield Focus"
        if row['ROE'] >= 12.0: return "Efficiency Leader"
        return "Fundamental Choice"
    else:
        if row['Yield'] >= 4.5: return "利回り重視：高還元銘柄"
        if row['ROE'] >= 12.0: return "効率重視：資本効率優良"
        return "基盤重視：安定優良銘柄"
...
def fetch_and_score(df, current_lang):
    ...
    # AIスコア計算 (無理に100に固定せず自然な分布へ)
    w_map = {'☀️': 1.0, '☁️': 0.5, '☔': 0.0}
    res_df['Score'] = np.round((res_df['ROE'] * 2.5) + (res_df['Yield'] * 3.5) - (res_df['Payout'] * 0.15) + (res_df['Trend'].map(w_map) * 15), 1)
...
v_payout = st.sidebar.slider(t["max_payout"], 0.0, 150.0, st.session_state.get("payout_x", 80.0), 0.1)
