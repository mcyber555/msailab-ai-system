...
def fetch_stock_data(tickers):
    ...
    # データが空の場合の予備カラム作成
    if not results:
        return pd.DataFrame(columns=['Ticker', '銘柄名', 'ROE(%)', '配当利回り(%)', '配当性向(%)', '現在値'])
...
# --- 5. メイン表示処理 ---
with st.spinner('データをリアルタイム解析中...'):
    raw_df = fetch_stock_data(TICKERS)
    scored_df = apply_ai_scoring(raw_df)

if not scored_df.empty and 'ROE(%)' in scored_df.columns:
    final_df = scored_df[
    ...
    with col2:
        ...
        if not final_df.empty:
            ...
            st.warning("条件に一致する銘柄が現在ありません。スライダーを調整してください。")
else:
    st.error("データの取得に失敗しました。しばらく時間を置いてから再試行してください。")
