...
LANG_MAP = {
    ...
    "English": {
        "title": "🛡️ AI Asset Analysis: MSAI-Alpha v5.5",
...
# --- 3. 厳選銘柄実名リスト (ダミーを排除し、主要100社規模を実名で構成) ---
@st.cache_data
def get_master_data(current_lang):
    # 日本を代表する一流企業の実名・実Tickerリスト
    raw_list = [
        ...
        ('8766.T', '東京海上', 'Tokio Marine', '保険', '☀️', 14.0, 3.5, 45.0, 3800),
        ('8591.T', 'オリックス', 'ORIX', '金融', '☀️', 9.8, 4.3, 33.0, 3240),
        ('9101.T', '日本郵船', 'NYK Line', '海運', '☀️', 12.0, 5.1, 30.0, 4800),
        ('1925.T', '大和ハウス', 'Daiwa House', '建設', '☁️', 11.0, 3.5, 35.0, 4200),
    ]
    stocks = []
    for r in raw_list:
...
def fetch_and_score(df):
    ...
    for _, row in df.iterrows():
        try:
            ...
            # 利回り補正ロジック：小数(0.04)を4.0%へ。異常値はバックアップへ
            if yld is not None:
                yld = float(yld)
                if yld < 0.2: yld *= 100 
                if yld > 30: yld = row['Y'] 
