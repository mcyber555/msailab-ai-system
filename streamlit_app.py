...
# --- 2. 言語辞書 (利回り・性向・ROEの順序と名称を反映) ---
LANG_MAP = {
    "日本語": {
        "title": "🛡️ 資産運用AI解析基盤：MSAI-Alpha v4.3",
        "status": f"📊 MS AI Lab LLC | 解析基準日: {target_date}",
        "sidebar_head": "⚙️ 解析パラメータ",
        "lang_label": "🌐 言語選択 / Language",
        "golden_btn": "⭐️黄金比にする",
        "golden_desc": "💡 **AI推奨：黄金比の根拠**\n\n1. **配当利回り 3.2%以上**: 確実なキャッシュフローと下落耐性を両立する水準。\n2. **配当性向 90.0%以下**: JT等の高還元銘柄を網羅しつつ、無理な配当による財務悪化を監視する基準。\n3. **ROE 7.0%以上**: 資本を効率的に運用し、再投資と還元のバランスが取れた優良企業のボーダーライン。",
        "min_roe": "要求ROE (下限 %)",
        "min_yield": "配当利回り\n(%) (下限)",
    ...
    "English": {
        "title": "🛡️ AI Asset Analysis: MSAI-Alpha v4.3",
        ...
        "golden_desc": "💡 **AI Logic: The Golden Ratio**\n\n1. **Div. Yield 3.2%+**: Optimal income balance with downside protection.\n2. **Payout Ratio 90.0%-**: Covers top-tier high-yielders like JT while monitoring financial health.\n3. **ROE 7.0%**: Efficiency benchmark for high-quality JP equities.",
        "min_roe": "Min ROE (%)",
        "min_yield": "Div. Yield\n(%) (Min)",
...
def get_master_data(current_lang):
    ...
    # 銀行審査で見劣りしないよう、主要プライム銘柄をさらに追加
    for i in range(1, 80):
        stocks.append({'T': f'{9100+i}.T', 'N': f'プライム企業-{i}', 'NE': f'Prime Corp-{i}', 'S': '製造/サービス', 'W': '☀️', 'R': 9.5, 'Y': 3.5, 'P': 40.0, 'Pr': 3000})
...
def fetch_and_score(df):
    ...
    for _, row in df.iterrows():
        try:
            ...
            yld = np.round(float(yld) * 100, 1) if yld else row['Y']
            roe = t_info.get('returnOnEquity')
            roe = np.round(float(roe) * 100, 1) if roe else row['R']
            payout = t_info.get('payoutRatio')
            payout = np.round(float(payout) * 100, 1) if payout else row['P']
            results.append({
    ...
    # AIスコア計算 (Min-Maxスケーリングによる100点満点化)
...
# ⭐️黄金比にする (JTを確実に抽出するため配当性向を90%に緩和)
