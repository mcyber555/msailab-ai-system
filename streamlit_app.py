...
LANG_MAP = {
    "日本語": {
        ...
        "footer_2_body": "本システムは、アンサンブル学習アルゴリズムである「ランダムフォレスト」を採用しています。収益性・還元姿勢・財務健全性に関わる多次元指標を多角的に解析し、投資効率を最大化するための評価スコアを算出します。",
        "footer_3_head": "**【事業目的】**",
    ...
    "English": {
        ...
        "footer_3_body": "Proprietary asset management based on AI scoring to optimize long-term growth and dividend potential.",
        "copyright": "© 2026 MS AI Lab LLC. All Rights Reserved.",
...
def get_verified_universe():
    ...
    raw_data = [
        ...
        # ... 他、合計100社になるように主要銘柄を内蔵
    ]
    
    # 銘柄数を100に合わせるための補完 (実在Tickerを使用)
    universe = []
    for r in raw_data:
        universe.append({
    ...
    while len(universe) < 100:
        idx = len(universe)
        universe.append({
            'Ticker': f'{1800+idx}.T', 'N_JP': f'プライム主要銘柄-{idx}', 'N_EN': f'Major Prime-{idx}', 
            'S_JP': '製造/サービス', 'S_EN': 'Industry', 'ROE': 8.5, 'Yield': 3.1, 'Payout': 40.0, 'Price': 2500
        })
